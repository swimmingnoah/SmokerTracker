...
|----------|---------|-------------|
| `INFLUX_URL` | `http://10.0.0.3:8086` | InfluxDB URL |
...
| `HA_URL` | `http://10.0.0.3:8123` | Home Assistant URL (future use) |
...
ssh root@10.0.0.3 'mkdir -p /mnt/user/appdata/smoker-app'

# 2. Transfer compose file and secrets
rsync -av docker-compose.yml root@10.0.0.3:/mnt/user/appdata/smoker-app/
rsync -av .env root@10.0.0.3:/mnt/user/appdata/smoker-app/
...
ssh root@10.0.0.3 'echo YOUR_GITHUB_PAT | docker login ghcr.io -u swimmingnoah --password-stdin'

# 4. Start the stack
ssh root@10.0.0.3 'cd /mnt/user/appdata/smoker-app && docker compose up -d'
```
...
For local dev, temporarily change `apiUrl` back to `http://10.0.0.3:5001/api`.
...
After deploying:
- [ ] `http://10.0.0.3:3001/` loads the session list
- [ ] `http://10.0.0.3:3001/api/health` returns `{"status": "ok"}`
...
- [ ] `http://10.0.0.3:5000/` and `:5001/` are both connection refused (backend not exposed)
